...
<div className='flex items-center bg-gray-700 rounded-full overflow-hidden shadow-xl  relative w-full'>
  <input type="text" className='flex-grow px-4 py-3 bg-transparent text-white placeholder-gray-400
  focus-outline-none text-sm sm:text-base' placeholder='What do you want to learn? (e.g. AI, MERN, Cloud...'/>
  <button className='absolute right-14 sm:right-16 bg-white rounded-full'><img className='cursor-pointer w-9 h-9 rounded-full p-2' src={pic} alt="" /></button>
  <button className='absolute right-2 bg-white rounded-full w-9 h-9 flex items-center justify-center '><RiMicAiFill className='cursor-pointer w-5 h-5 text-[#cb87c5]'/></button>
</div>
</div> 
    </div>
  )
